...
# return to routes.py
#[a] add login_required to the list of modules to be imported from flask_login
#[0]: go to the account route @app.route('/account')
#[b]: simply add the @login_required decorator below the existing decorator:

'''
[a] from flask_login import login_required
[0] @app.route('/account')
[b] @login_required
[1] def account():
[2]     return render_template('account.html', title='Account')
'''
#n.b. [1][2] are already-existing code to illustrate the changes
...
#[b]: and just add another line 'login_manager.login_message_category',
